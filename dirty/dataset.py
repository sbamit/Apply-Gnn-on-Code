...
class Example:
    ...
    def from_json(cls, d: Dict):
        print('\nsource:', d['source'], '\ntarget:', d['target'])
        src_code = " ".join(d["code_tokens"])
        print('src code:\n', Example.format_c_program(src_code))
        print('func_name:', d["name"], '#', next(iid))
        source = {
    ...
    @staticmethod
    def format_c_program(inline_c_program):
        lines = inline_c_program.strip().split(';')
        indentation_level = 0
        formatted_lines = []
        ...
        for line in lines:
            ...
            if line.endswith("{"):
                formatted_lines.append('\t' * indentation_level + line + "\n")
                indentation_level += 1
...
class Dataset(wds.WebDataset):
    ...
    def __init__(self, url: str, config: Optional[Dict] = None, percent: float = 1.0):
        ...
        if config:
            ...
            sort = identity
        self = (
            self.compose(Dataset._file_iter_to_line_iter)
